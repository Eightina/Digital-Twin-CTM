...
from webster import phase4webster, best_duration, ga_best_duration, bo_best_duration
...
def ac_p81(
    ...
    # search_backup_num=8,
    # delay_units = 1,
    # delay_seconds_per_unit = 5.0,
    # max_iter=20,
    # size_pop = 10
):
    # delay_units = max_iter
    ...
    for cycle in range(total_cycles):
        ...
        if cycle >= start_offset_cycles:  # 2 ~ 11
            ...
            cur_duration = phase4webster(cur_demand, num = 1)
            set_sumo_logic("0", cur_duration)
            cur_cycle_len = round(sum(cur_duration) + phase_num * 4)
        #     pred_demand = []
        #     for i in range(4):
        ...
        #         pred_demand = [int(_) for _ in pred_demand]
        ...
        #     #     file_name="Test6"
        #     # )
        #     [delay0, cur_duration] = bo_best_duration(
        #         step_veh,
        #         arc_demand=pred_demand,
        #         # num=search_backup_num,
        #         cell_scale=cell_scale,
        #         # ctm_coeff=ctm_coeff,
        #         sim_cycle_num=sim_cycle_num,
...
sumo_delay, time0 = ac_p81(
    ...
    # sumo_df.insert()
sumo_df["{}".format('adaptive')] = list(sumo_delay)
# ctm_df["{}".format(max_iter)] = list(og_ctm_delay)
time_df["{}".format('adaptive')] = list(time0)
sumo_df.to_csv("./Output/experiments/8/1/sumo.csv")
# ctm_df.to_csv("./Output/experiments/7/x/ctm.csv")
time_df.to_csv("./Output/experiments/8/1/time.csv")
