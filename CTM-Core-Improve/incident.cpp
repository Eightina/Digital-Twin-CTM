...
incident::incident(int aa, float dist, int st, int et, float mf) {}
	...
	//float len =arcs[at_arc].get_cell_length();
	//at_cell = (int)ceil(distance / len) + arcs[at_arc].get_first_cell() - 1;
	//start_clock = (int)floor((start_time - settings.start_time) * 1.0 / owner->settings.clock_tick) + 1;
	//end_clock = (int)floor((end_time - settings.start_time) * 1.0 / owner->settings.clock_tick) + 1;
	//id = size + 1;
	//size++;
